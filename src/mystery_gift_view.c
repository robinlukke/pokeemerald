...
static void DrawCardWindow(u8 whichWindow)
{
    ...
    case CARD_WIN_HEADER:
    {
        ...
        s32 x;
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, 0, 1, sCard_TextColorTable[sWonderCardData->gfx->titleTextPal], 0, sWonderCardData->titleText);
        x = 160 - GetStringWidth(FONT_NORMAL, sWonderCardData->subtitleText, GetFontAttribute(FONT_NORMAL, FONTATTR_LETTER_SPACING));
        if (x < 0)
            x = 0;
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, x, 17, sCard_TextColorTable[sWonderCardData->gfx->titleTextPal], 0, sWonderCardData->subtitleText);
    ...
    case CARD_WIN_BODY:
        ...
        for (; i < WONDER_CARD_BODY_TEXT_LINES; i++)
            AddTextPrinterParameterized3(windowId, FONT_NORMAL, 0, 16 * i + 2, sCard_TextColorTable[sWonderCardData->gfx->bodyTextPal], 0, sWonderCardData->bodyText[i]);
        break;
    case CARD_WIN_FOOTER:
        ...
        else
        {
            ...
            {
                // Print stat text
                AddTextPrinterParameterized3(windowId, FONT_NORMAL, x, y, sCard_TextColorTable[sWonderCardData->gfx->footerTextPal], 0, sWonderCardData->statTextData[i].statText);
...
static void DrawNewsWindows(void)
{
    ...
    if (x < 0)
        x = 0;
    AddTextPrinterParameterized3(sWonderNewsData->windowIds[NEWS_WIN_TITLE], FONT_NORMAL, x, 6, sNews_TextColorTable[sWonderNewsData->gfx->titleTextPal], 0, sWonderNewsData->titleText);

    // Print body text
    for (; i < WONDER_NEWS_BODY_TEXT_LINES; i++)
        AddTextPrinterParameterized3(sWonderNewsData->windowIds[NEWS_WIN_BODY], FONT_NORMAL, 0,
                                     16 * i + 2,
